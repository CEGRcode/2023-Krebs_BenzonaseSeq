...
def count_rows_based_on_column(input_file, output_file):
    ...
    # Count the number of rows where the value in column 2 is greater than 501
    count = (df.iloc[:, column_index] > 501).sum()
    
    # Write the count to the output file with the updated header message
    with open(output_file, 'w') as f:
        f.write(f"Unique, significant peaks 3' to motif: {count}\n")

if __name__ == "__main__":
    # Set up command-line argument parsing
    parser = argparse.ArgumentParser(description="Count rows where values in column 2 are greater than 501.")
