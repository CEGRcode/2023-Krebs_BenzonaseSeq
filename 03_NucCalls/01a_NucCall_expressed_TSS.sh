...
#output directory
OUTPUT=/gpfs/group/bfp2/default/pughlab-members/wkl2-WillLai/NucleosomeAtlas_Project/230720_plus1_minus1/output_v2_NonRed_Oct_Hex_Tet_230825
...
TSS=/gpfs/group/bfp2/default/pughlab-members/wkl2-WillLai/NucleosomeAtlas_Project/230720_plus1_minus1/K562_CoPRO-expressed_Gene-refSeqTSS_2000bp.bed
...
cat K562_CoPRO_TSS_upstreamOctamers_Intersect.bed | awk '{if ($6=="+") $14=sqrt(((($8+$9)/2)-$3)*((($8+$9)/2)-$3)); else $14=sqrt(((($8+$9)/2)-$2)*((($8+$9)/2)-$2)); print}' | sort -k4,4d -k14,14n -r | sort -k4,4d -u | sort -k14,14n -r | awk '{print $7"\t"$8"\t"$9"\t"$10"\t"$11"\t"$6}' > K562_Minus1_SORTdistanceToTSS_nonRedOct_Hex_Tet.bed

#same as above but with TSS ID in 7th column
cat K562_CoPRO_TSS_upstreamOctamers_Intersect.bed | awk '{if ($6=="+") $14=sqrt(((($8+$9)/2)-$3)*((($8+$9)/2)-$3)); else $14=sqrt(((($8+$9)/2)-$2)*((($8+$9)/2)-$2)); print}' | sort -k4,4d -k14,14n -r | sort -k4,4d -u | sort -k14,14n -r | awk '{print $7"\t"$8"\t"$9"\t"$10"\t"$11"\t"$6"\t"$4}' > K562_Minus1_SORTdistanceToTSS_nonRedOct_Hex_Tet.tsv

#now sort output nuclesomes by #_TSS to revert back to decreasing RNA_expression
cat K562_CoPRO_TSS_upstreamOctamers_Intersect.bed | awk '{if ($6=="+") $14=sqrt(((($8+$9)/2)-$3)*((($8+$9)/2)-$3)); else $14=sqrt(((($8+$9)/2)-$2)*((($8+$9)/2)-$2)); print}' | sort -k4,4d -k14,14n -r | sort -k4,4d -u | sort -k4,4n -r | awk '{print $7"\t"$8"\t"$9"\t"$10"\t"$11"\t"$6}' > K562_Minus1_SORTbyRNAexp_nonRedOct_Hex_Tet.bed
...
#fixed so that midpoint of dyad is taken or (($8+$9)/2) instead of $8
cat K562_CoPRO_TSS_downstreamOctamers_Intersect.bed | awk '{if ($6=="+") $14=sqrt(((($8+$9)/2)-$2)*((($8+$9)/2)-$2)); else $14=sqrt(((($8+$9)/2)-$3)*((($8+$9)/2)-$3)); print}' | sort -k4,4d -k14,14n -r | sort -k4,4d -u | sort -k14,14n -r | awk '{print $7"\t"$8"\t"$9"\t"$10"\t"$11"\t"$6}' > K562_Plus1_SORTdistanceToTSS_nonRedOct_Hex_Tet.bed

#same as above but with TSS ID in 7th column
cat K562_CoPRO_TSS_downstreamOctamers_Intersect.bed | awk '{if ($6=="+") $14=sqrt(((($8+$9)/2)-$2)*((($8+$9)/2)-$2)); else $14=sqrt(((($8+$9)/2)-$3)*((($8+$9)/2)-$3)); print}' | sort -k4,4d -k14,14n -r | sort -k4,4d -u | sort -k14,14n -r | awk '{print $7"\t"$8"\t"$9"\t"$10"\t"$11"\t"$6"\t"$4}' > K562_Plus1_SORTdistanceToTSS_nonRedOct_Hex_Tet.tsv

#now sort output nuclesomes by #_TSS to revert back to decreasing RNA_expression
cat K562_CoPRO_TSS_downstreamOctamers_Intersect.bed | awk '{if ($6=="+") $14=sqrt(((($8+$9)/2)-$2)*((($8+$9)/2)-$2)); else $14=sqrt(((($8+$9)/2)-$3)*((($8+$9)/2)-$3)); print}' | sort -k4,4d -k14,14n -r | sort -k4,4d -u | sort -k4,4n -r | awk '{print $7"\t"$8"\t"$9"\t"$10"\t"$11"\t"$6}' > K562_Plus1_SORTbyRNAexp_nonRedOct_Hex_Tet.bed
