# purpose - this code uses the +1 bedfile to calculate the number of tags (representing SNs) in the proximal and distal SNs regions (based on midpoints in Fig. 4). The final files gives the number of rows with proximal SNs, distal SNs, or both at +1 positions that are based on all +1 nucleosomes (as compared to thus based only on full-length nucleosomes.
...
#output directory
OUTPUT=/storage/group/bfp2/default/juk398-JordanKrebs/NucleosomeAtlas_project/figures/Fig4_transcription/H3K4me3_SNs_Plus1_all_JEK_240121
...
BEDFILE_2bp=$(echo $Plus1_BEDFILE | rev | cut -d"/" -f1 | rev | awk -F. '{print $1"_2bp.bed"}')
...
sampleID=Fig4_H3K4me3_SNs_+1_all_JEK_240120.slurm
rm -f $sampleID
echo "$JOBSTATS" >> $sampleID
echo "#set directory" >> $sampleID
echo "cd $OUTPUT" >> $sampleID
echo "#expand bedfile" >> $sampleID
echo "java -jar $SCRIPTMANAGER coordinate-manipulation expand-bed -c=2 $Plus1_BEDFILE -o=$BEDFILE_2bp" >> $sampleID
...
echo "paste $Plus1_proximal_SN $Target_proximal  |  awk '(\$4==\$7){print \$0}' | awk '(\$8!=0){print \$0}' | wc -l | awk '{print \"proximal_SN_region_withSNs\"\"\t\"\$1}' > $NUMBERS_SNs" >> $sampleID
echo "paste $Plus1_proximal_SN $Target_proximal  |  awk '(\$4==\$7){print \$0}' | awk '(\$8==0){print \$0}' | wc -l | awk '{print \"proximal_SN_region_withoutSNs\"\"\t\"\$1}' >> $NUMBERS_SNs" >> $sampleID
echo "paste $Plus1_distal_SN $Target_distal  |  awk '(\$4==\$7){print \$0}' | awk '(\$8!=0){print \$0}' | wc -l | awk '{print \"distal_SN_region_withSNs\"\"\t\"\$1}' >> $NUMBERS_SNs" >> $sampleID
echo "paste $Plus1_distal_SN $Target_distal  |  awk '(\$4==\$7){print \$0}' | awk '(\$8==0){print \$0}' | wc -l | awk '{print \"distal_SN_region_withoutSNs\"\"\t\"\$1}' >> $NUMBERS_SNs" >> $sampleID
echo "paste $BEDFILE_2bp $Target_proximal $Target_distal  |  awk '(\$4==\$7 && \$4==\$9){print \$0}' | awk '(\$8!=0 && \$10!=0){print \$0}' | wc -l | awk '{print \"both_SNs_present\"\"\t\"\$1}' >> $NUMBERS_SNs" >> $sampleID
echo "paste $BEDFILE_2bp $Target_proximal $Target_distal  |  awk '(\$4==\$7 && \$4==\$9){print \$0}' | wc -l | awk '{print \"total_possible_for_combined\"\"\t\"\$1}' >> $NUMBERS_SNs" >> $sampleID
