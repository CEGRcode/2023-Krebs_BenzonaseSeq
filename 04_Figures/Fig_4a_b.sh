# purpose - code for Fig. 3a_b updated to have insert sizes of 128-164 to show full-length nucleosomal reads for BNase-ChIP-exo. This version uses all sites for the composite plots.
...
#output directory
OUTPUT=/storage/group/bfp2/default/juk398-JordanKrebs/NucleosomeAtlas_project/figures/nre_fig3_240219/fig3a_b_ChIPs_+1_128to164_output_240305
...
BAM12=/storage/group/bfp2/default/juk398-JordanKrebs/NucleosomeAtlas_project/230810_ChIPs/MERGED_datasets/25754_25864_25872_28458_28466_Benz_0sonicCycles_BX_CTCF_master.bam
BAM13=/storage/group/bfp2/default/juk398-JordanKrebs/NucleosomeAtlas_project/230810_ChIPs/MERGED_datasets/IgG_9reps_Benz_0sonicCycles_BX_IgG_master.bam
...
BAM13a=$(echo $BAM13 | rev | cut -d"/" -f1 | rev | awk -F. '{print $1}')
OUT13=$(echo "$BAM13a""_""$BEDFILE_2000" | awk -F. '{print $1"_midpoint.out"}')
CDT13=$(echo "$BAM13a""_""$BEDFILE_2000" | awk -F. '{print $1}')
CDT13b=$(echo "$BAM13a""_""$BEDFILE_2000" | awk -F. '{print $1"_combined.cdt.gz"}')
CDT13c=$(echo "$BAM13a""_""$BEDFILE_2000" | awk -F. '{print $1"_combined.cdt"}')
CDT13_SCALED=$(echo "$BAM13a""_""$BEDFILE_2000" | awk -F. '{print $1"_scaled.cdt"}')
SCALE13=$(echo "$BAM13a" | awk -F. '{print $1"_ForCDT"}')
SCALE13a=$(echo "$BAM13a" | awk -F. '{print $1"_ForCDT_ScalingFactors.out"}')
PNG13=$(echo "$BAM13a""_""$BEDFILE_2000" | awk -F. '{print $1"_scaled.png"}')
SVG13=$(echo "$BAM13a""_""$BEDFILE_2000" | awk -F. '{print $1"_scaled_labeled.svg"}')
...
OUT13a_3000=$(echo "$BAM13a""_""$BEDFILE_3000" | awk -F. '{print $1"_ForComposite_midpoint.out"}')
CDT13d_3000=$(echo "$BAM13a""_""$BEDFILE_3000" | awk -F. '{print $1"_ForComposite"}')
CDT13e_3000=$(echo "$BAM13a""_""$BEDFILE_3000" | awk -F. '{print $1"_ForComposite_combined.cdt.gz"}')
CDT13f_3000=$(echo "$BAM13a""_""$BEDFILE_3000" | awk -F. '{print $1"_ForComposite_combined.cdt"}')
CDT13_SCALED_COMP_3000=$(echo "$BAM13a""_""$BEDFILE_3000" | awk -F. '{print $1"_ForComposite_scaled.cdt"}')
SCALED_OUT13_3000=$(echo "$BAM13a""_""$BEDFILE_3000" | awk -F. '{print $1"_ForComposite_scaled.tab"}')

sampleID=fig3a_b_ChIPs_+1_128to164bp_240305_ROAR.slurm
rm -f $sampleID
...
echo "java -jar $SCRIPTMANAGER read-analysis scaling-factor -t --blacklist=$BLACKLIST -o=$SCALE13 $BAM13" >> $sampleID
...
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT8 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT8 --min-insert=128 --max-insert=164 $BEDFILE_2000 $BAM8" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT9 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT9 --min-insert=128 --max-insert=164 $BEDFILE_2000 $BAM9" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT10 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT10 --min-insert=128 --max-insert=164 $BEDFILE_2000 $BAM10" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT11 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT11 --min-insert=128 --max-insert=164 $BEDFILE_2000 $BAM11" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT12 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT12 --min-insert=128 --max-insert=164 $BEDFILE_2000 $BAM12" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT13 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT13 --min-insert=128 --max-insert=164 $BEDFILE_2000 $BAM13" >> $sampleID
...
echo "gunzip -c $CDT13b > $CDT13c" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis scale-matrix -o=$CDT13_SCALED --scaling-factor=\$(cat $SCALE13a | cut -f2 | tail -1 | awk '{print \$1}') $CDT13c" >> $sampleID
...
echo "java -Djava.awt.headless=true -jar $SCRIPTMANAGER figure-generation heatmap -o=$PNG12 -c 703000 -p=0.95 $CDT12_SCALED" >> $sampleID
echo "java -Djava.awt.headless=true -jar $SCRIPTMANAGER figure-generation heatmap -o=$PNG13 -c 000000 -p=0.95 $CDT13_SCALED" >> $sampleID
...
echo "java -Djava.awt.headless=true -jar $SCRIPTMANAGER figure-generation label-heatmap $PNG8 --output=$SVG8 --width=2 --font-size=18 --left-label='-1' --mid-label=0 --right-label='+1' --x-label='Distance from +1 Dyad (kb)' --y-label='11,714 Nucleosome positions'" >> $sampleID
echo "java -Djava.awt.headless=true -jar $SCRIPTMANAGER figure-generation label-heatmap $PNG9 --output=$SVG9 --width=2 --font-size=18 --left-label='-1' --mid-label=0 --right-label='+1' --x-label='Distance from +1 Dyad (kb)' --y-label='11,714 Nucleosome positions'" >> $sampleID
echo "java -Djava.awt.headless=true -jar $SCRIPTMANAGER figure-generation label-heatmap $PNG10 --output=$SVG10 --width=2 --font-size=18 --left-label='-1' --mid-label=0 --right-label='+1' --x-label='Distance from +1 Dyad (kb)' --y-label='11,714 Nucleosome positions'" >> $sampleID
echo "java -Djava.awt.headless=true -jar $SCRIPTMANAGER figure-generation label-heatmap $PNG11 --output=$SVG11 --width=2 --font-size=18 --left-label='-1' --mid-label=0 --right-label='+1' --x-label='Distance from +1 Dyad (kb)' --y-label='11,714 Nucleosome positions'" >> $sampleID
echo "java -Djava.awt.headless=true -jar $SCRIPTMANAGER figure-generation label-heatmap $PNG12 --output=$SVG12 --width=2 --font-size=18 --left-label='-1' --mid-label=0 --right-label='+1' --x-label='Distance from +1 Dyad (kb)' --y-label='11,714 Nucleosome positions'" >> $sampleID
echo "java -Djava.awt.headless=true -jar $SCRIPTMANAGER figure-generation label-heatmap $PNG13 --output=$SVG13 --width=2 --font-size=18 --left-label='-1' --mid-label=0 --right-label='+1' --x-label='Distance from +1 Dyad (kb)' --y-label='11,714 Nucleosome positions'" >> $sampleID
...
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT8d_3000 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT8a_3000 --min-insert=128 --max-insert=164 $BEDFILE_3000 $BAM8" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT9d_3000 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT9a_3000 --min-insert=128 --max-insert=164 $BEDFILE_3000 $BAM9" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT10d_3000 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT10a_3000 --min-insert=128 --max-insert=164 $BEDFILE_3000 $BAM10" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT11d_3000 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT11a_3000 --min-insert=128 --max-insert=164 $BEDFILE_3000 $BAM11" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT12d_3000 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT12a_3000 --min-insert=128 --max-insert=164 $BEDFILE_3000 $BAM12" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis tag-pileup -m -z --combined --output-matrix=$CDT13d_3000 -N -p --cpu=4 --blacklist-filter=$BLACKLIST -o=$OUT13a_3000 --min-insert=128 --max-insert=164 $BEDFILE_3000 $BAM13" >> $sampleID
echo "#scale data in matrix by scaling factor" >> $sampleID
...
echo "gunzip -c $CDT13e_3000 > $OUTPUT/$CDT13f_3000" >> $sampleID
echo "java -jar $SCRIPTMANAGER read-analysis scale-matrix -o=$CDT13_SCALED_COMP_3000 --scaling-factor=\$(cat $SCALE13a | cut -f2 | tail -1 | awk '{print \$1}') $CDT13f_3000" >> $sampleID
...
echo "perl $JOB $CDT13_SCALED_COMP_3000 $SCALED_OUT13_3000" >> $sampleID
...
echo "rm $OUT13" >> $sampleID
echo "rm $CDT13b" >> $sampleID
echo "rm $CDT13c" >> $sampleID
echo "rm $CDT13_SCALED" >> $sampleID
echo "rm $SCALE13a" >> $sampleID
echo "rm $PNG13" >> $sampleID
...
echo "rm $OUT13a_3000" >> $sampleID
echo "rm $CDT13e_3000" >> $sampleID
echo "rm $CDT13f_3000" >> $sampleID
echo "rm $CDT13_SCALED_COMP_3000" >> $sampleID
